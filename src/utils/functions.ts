import { LandInterface, LogInterface, PlayerInterface } from "./data.types"
...
export const checkMonopoly = (lands: LandInterface[], land: LandInterface, player: PlayerInterface) => {
    const setLands = lands.filter(_land => _land.setID === land.setID)
    
    const setLandsIDs = setLands.map(ld => ld.setID)

   const playerLands = player.lands.map(_landID => getLandFromID(_landID, lands))
   
   const playerLandsInSet = playerLands.filter(_land => _land.setID === land.setID)

   let hasMonopoly = setLandsIDs.length === playerLandsInSet.length
    
    return hasMonopoly
}

export const buildCash = (land: LandInterface) => {
    return (land.price / 2)
}
...
export const getCharacterNotSelected = (selected: string[], characters: string[]) =>
 ...
 export const convertLogsTimestamp = (logs: LogInterface[]) => {
    let newLogs = [...logs]
    newLogs[0] = {
    ...
    for (let i = 1; i < logs.length; i++) {
        newLogs[i] = {
            ...
            timestamp: logs[i].timestamp - logs[i - 1].timestamp
        }
    }
...
export function formatTime(_stamp: number) {
    let correctStamp = _stamp / 1000
  var hours = Math.floor(correctStamp / 60 / 60)

  var minutes = Math.floor(correctStamp / 60) - (hours * 60)
  var seconds = Math.floor(correctStamp % 60)
