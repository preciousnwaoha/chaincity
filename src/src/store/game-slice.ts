...
const gameSlice = createSlice( {
    ...
    reducers: {
        ...
        build(state, action: PayloadAction<{land: LandInterface, player: number}>) {
            let updatedLands = [...state.lands]
            let playerObj = state.players[action.payload.player]
            let indexOfLand = updatedLands.indexOf(action.payload.land)

            
            let updatedLand = {
                ...action.payload.land,
                houses: action.payload.land.houses += 1
            }
            updatedLands = updatedLands.filter(_land => _land.id !== action.payload.land.id)
            let a = updatedLands.slice(0, indexOfLand)
            let b = updatedLands.slice(indexOfLand)

            updatedLands = [...a, updatedLand, ...b]
            state.lands = updatedLands
            state.logs = [...state.logs, {
                message: `${playerObj.name} bought 1 house at ${updatedLand.name}`,
                timestamp: new Date().getTime()
            }]
        },
        sell(state, action: PayloadAction<{land: LandInterface, player: number}>) {
            let updatedLands = [...state.lands]
            let updatedPlayers = [...state.players]
            let playerObj = state.players[action.payload.player]
            let indexOfLand = updatedLands.indexOf(action.payload.land)
            
            let updatedLand = {...action.payload.land}
            updatedLand.houses -= 1
            updatedLands = updatedLands.filter(_land => _land.id !== action.payload.land.id)
            let a = updatedLands.slice(0, indexOfLand)
            let b = updatedLands.slice(indexOfLand)
            ...
            if (updatedLand.houses === 0) {
                

                playerObj.lands = playerObj.lands.filter(id => id !== action.payload.land.id)
                let a = updatedPlayers.slice(0, action.payload.player)
                let b = updatedPlayers.slice(action.payload.player)
            ...
            state.logs = [...state.logs, {
                message: `${playerObj.name} sold 1 house at ${updatedLand.name}`,
                timestamp: new Date().getTime()
            }]
        },
